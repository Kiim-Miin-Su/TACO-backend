...
export class LoggingInterceptor implements NestInterceptor {
  ...
  intercept(ctx: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const path = safeUrlForLog(originalUrl);
    ...
    return next.handle().pipe(
      tap({
        next: () => {
          ...
          this.logger.log(logLine('http', { method, path, status: res.statusCode, ms: Date.now() - started }));
        },
        error: (err: { status?: number; message?: string }) => {
          // 에러 응답 본문·처리는 AllExceptionsFilter(category=error) 담당 — 여기선 http 계측만
          this.logger.warn(logLine('http', { method, path, status: err?.status ?? 500, ms: Date.now() - started, message: err?.message }));
        },
      }),
    );
  }

...
export class PaymentsService implements OnModuleInit {
  ...
  async update(id: number, dto: UpdatePaymentDto, actorId?: number): Promise<Payment> {
    return this.unitOfWork.run(async () => {
      const row = { ...this.findOne(id) }; // live-reference 함정 방지(diff용 클론)
      ...
      const after = await this.store.update<Payment>(PAYMENTS_SPEC, id, { ...dto }) as Payment;
      // [감사 전수 2026-07-16] 청구 정정 diff 이력.
      if (actorId != null) {
    ...
    return this.unitOfWork.run(async () => {
      ...
      if (actorId != null) {
        await this.audit.log({
          ...
          changes: { status: { before: row.status, after: 'paid' }, paidAmount: { after: row.amount } },
        });
        await this.audit.log({

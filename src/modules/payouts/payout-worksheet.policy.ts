...
//
//  분류(대표 지시 ⑤·⑦·⑧):
//   · auto     — held ∧ 강사 출결 ∈ {출석·미표시·보강} ∧ 전 코호트 리포트 승인 ∧ 시급>0
//                → 기본값 = 시급×시간(payoutAmountOf). 책정가(override)가 있으면 책정가 우선.
//   · manual   — held ∧ 결석 아님 ∧ (지각 ∨ 리포트 미작성/미승인/반려 ∨ roster 없음 ∨ 시급 미설정)
//                → 책정 전 amount=null(빈칸 — 합계 제외), 매니저/대표가 책정하면 포함.
...
export function classifySessionForPayout(session: WorksheetSession, input: ClassifyInput): WorksheetClassification {
  ...
  else if (!reportsComplete(input.participantIds, input.reportOf)) manualReasons.push('report_incomplete');

...
export class PayoutsService implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    this.pay(paid.id);
    console.log('[payouts.seedDemo] 강사1 적격 3건(미정산) · 강사2 지급완료 1건 주입');
  }
  ...
  generate(instructorId: number, from: string, to: string): InstructorPayoutRow {
    ...
    console.log('[payouts.generate]', JSON.stringify({ instructorId, from, to, sessionCount: m.sessionCount, amount: m.computedAmount }));
    return payout;
  }
  ...
  pay(id: number): { payout: InstructorPayoutRow; transaction: TransactionRow } {
    ...
    console.log('[payouts.pay]', JSON.stringify({ id, amount: payout.amount, txId: transaction.id }));

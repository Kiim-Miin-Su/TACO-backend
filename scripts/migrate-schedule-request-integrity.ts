...
import {
  ...
  SCHEDULE_REQUEST_INTEGRITY_SQL,
...
async function main(): Promise<void> {
  ...
  await dataSource.query(SCHEDULE_REQUEST_INTEGRITY_SQL[0]);
  if (!apply) {
  ...
  try {
    await runner.query('SELECT pg_advisory_lock($1, $2)', [77, 30]);
    ...
    try {
      ...
      if (!applied.length) {
        for (const sql of SCHEDULE_REQUEST_INTEGRITY_SQL) await runner.query(sql);
        await runner.query(
    ...
    try {
      await runner.query('SELECT pg_advisory_unlock($1, $2)', [77, 30]);
    } finally {
  ...
  if (
    after.constraints.length !== SCHEDULE_REQUEST_INTEGRITY_CONSTRAINTS.length
    || !after.constraints.every((row: { convalidated: boolean }) => row.convalidated)
    || !after.ledgerApplied
  ) {
    throw new Error('schedule request integrity migration readback failed');
  }
  console.log(JSON.stringify({ ok: true, migration: SCHEDULE_REQUEST_INTEGRITY_MIGRATION_ID, after }, null, 2));
}

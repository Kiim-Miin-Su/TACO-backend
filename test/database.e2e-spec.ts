...
describe('InMemoryDatabase — transaction & index', () => {
  ...
  it('트랜잭션 커밋: fn 정상 완료 시 모든 쓰기가 반영된다', () => {
  ...
  it('트랜잭션 롤백: 중간 예외 시 insert/update/remove·시퀀스까지 전부 원복(부분 쓰기 잔존 금지)', () => {
  ...
  it('롤백 후에도 인덱스가 스냅샷 데이터와 정합(findByField 재조회)', () => {

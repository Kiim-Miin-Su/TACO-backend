...
describe('Schedule API (e2e)', () => {
  ...
  it('PATCH /schedule/:id — 시리즈 전체(scope=all) 동반 이동(updated>1)', async () => {
    ...
    const res = await http.patch(`/api/schedule/${target.id}`).set(TH())
  ...
  it('memo 왕복: POST로 생성 시 저장 + PATCH로 수정', async () => {
    const created = (await http.post('/api/schedule').set(TH())
  ...
  it('color 왕복: POST 색 저장 + PATCH 색 변경 / 미지정 시 코스 색 폴백', async () => {
    ...
    const fallback = (await http.post('/api/schedule').set(TH())
  ...
  it('충돌 시각(시드 월 16:00)을 취소로 변경 시 force 없이도 허용(충돌 무관)', async () => {
    ...
    const occupy = (await http.post('/api/schedule').set(TH())
      .send({ courseId: 10, sessionDate: MON, startTime: '16:00', durationMinutes: 90, force: true })
      .expect(201)).body.row;
    // 이 세션을 '취소(no_show)'로 변경 — 겹침이 있어도 409 없이 200
    const res = (await http.patch(`/api/schedule/${occupy.id}`).set(TH())
      .send({ status: 'no_show' })
      .expect(200)).body;
  ...
  it('결강(canceled) 세션은 시간 점유에서 제외 → 같은 슬롯 생성 가능', async () => {
    ...
    const ok = (await http.post('/api/schedule').set(TH())
      .send({ courseId: 10, sessionDate: day, startTime: '10:00', durationMinutes: 60 })
      .expect(201)).body;
  ...
  it('인가: 강사 토큰으로 세션 생성 → 201(로그인이면 역할 무관 허용)', async () => {
    ...
    await http.post('/api/schedule').set({ Authorization: `Bearer ${login.body.accessToken}` })

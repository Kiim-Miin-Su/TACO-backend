...
describe('Roadmaps API (e2e)', () => {
  ...
  it('GET /roadmaps — 시드 1건', async () => {
    const rows = (await http.get('/api/roadmaps').set(asAdmin()).expect(200)).body;
    expect(rows.length).toBeGreaterThanOrEqual(1);
    expect(rows[0]).toMatchObject({ id: 1, title: 'SAT 종합 로드맵', isActive: true });
  });

  it('GET /roadmaps/courses — 링크 2건, 코스 FK·순서 정합', async () => {
    const links = (await http.get('/api/roadmaps/courses').set(asAdmin()).expect(200)).body;
  ...
  it('POST /roadmaps — 없는 courseId → 400, 부분 생성 없음', async () => {
    const before = (await http.get('/api/roadmaps').set(asAdmin()).expect(200)).body.length;
    const beforeLinks = (await http.get('/api/roadmaps/courses').set(asAdmin()).expect(200)).body.length;
    ...
    expect((await http.get('/api/roadmaps').set(asAdmin()).expect(200)).body.length).toBe(before);
    expect((await http.get('/api/roadmaps/courses').set(asAdmin()).expect(200)).body.length).toBe(beforeLinks);
  });

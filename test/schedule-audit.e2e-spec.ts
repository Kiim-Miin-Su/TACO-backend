...
describe('세션 audit diff 노이즈 정리 (e2e)', () => {
  ...
  it('시드 세션 강사출결만 변경 → audit changes에 instructorAttendance만(미변경 kind/mode 없음)', async () => {
    ...
    const blocked = await http.put(`/api/schedule/${sid}/instructor-attendance`).set(auth()).send({ status: 'late' }).expect(409);
    expect(blocked.body.code).toBe('ACCOUNTING_IMPACT_ACK_REQUIRED');
    await http.put(`/api/schedule/${sid}/instructor-attendance`).set(auth()).send({
      status: 'late',

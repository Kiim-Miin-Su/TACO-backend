...
describe('[TBO-64] 시수 워크시트 (e2e)', () => {
  ...
  it('분류·기본값 — auto=시급×시간, 지각·리포트 미작성=빈칸(manual), 결석·미진행=excluded', async () => {
    ...
    expect(auto.participants[0]).toMatchObject({ studentId: 1, attendance: 'present', reportApproval: 'approved' });
    ...
    expect(norep.participants[0].reportApproval).toBeNull(); // 미작성 표시
  ...
  it('가격 책정(⑧) — 지각·리포트 미작성에 금액 확정 → 합계 반영, 해제(null) → 빈칸 복귀', async () => {
    await http.put(`/api/schedule/${LATE}/pay-amount`).set(auth('manager')).send({ amount: 70000 }).expect(200);
    await http.put(`/api/schedule/${NOREP}/pay-amount`).set(auth('manager')).send({ amount: 50000 }).expect(200);
    ...
    // 해제 → 빈칸 복귀(합계 제외)
    await http.put(`/api/schedule/${NOREP}/pay-amount`).set(auth('manager')).send({ amount: null }).expect(200);
    ws = await worksheet();
    expect(ws.totals).toMatchObject({ includedCount: 2, totalAmount: 170000, unpricedCount: 1 });
    // 가드: 결석·미진행 회차는 책정 불가, 음수 400, 강사 403
    await http.put(`/api/schedule/${ABSENT}/pay-amount`).set(auth('manager')).send({ amount: 10000 }).expect(400);
    await http.put(`/api/schedule/${SCHED}/pay-amount`).set(auth('manager')).send({ amount: 10000 }).expect(400);
    await http.put(`/api/schedule/${LATE}/pay-amount`).set(auth('manager')).send({ amount: -1 }).expect(400);
  ...
  it('generate 정합(단일 진실원) — 확정 라인(auto+책정)만 정산, 미책정 잔존 → 연결 후 가격 불변', async () => {
    ...
    await http.put(`/api/schedule/${LATE}/pay-amount`).set(auth('manager')).send({ amount: 90000 }).expect(409);
    const ws = await worksheet();

...
describe('Availability API (e2e)', () => {
  ...
  it('겹침 방지(버그2): 같은 오너·요일 겹치는 블록 → 409, 인접(안 겹침)은 통과', async () => {
    ...
    await http.put('/api/availability')
      .set(TH())
      .send({ ownerType: 'student', ownerId: 4, kind: 'unavailable', weekday: 2, startTime: '10:00', endTime: '11:00' })
      .expect(200);
    // 겹치는 시간(10:30–11:30) → 409 + 겹친 시각 메시지
    const clash = await http.put('/api/availability')
      .set(TH())
      .send({ ownerType: 'student', ownerId: 4, kind: 'unavailable', weekday: 2, startTime: '10:30', endTime: '11:30' })
      .expect(409);
    expect(JSON.stringify(clash.body)).toContain('겹칩니다');
    // 인접(11:00–12:00, 겹치지 않음) → 통과
    await http.put('/api/availability')
      .set(TH())
      .send({ ownerType: 'student', ownerId: 4, kind: 'available', weekday: 2, startTime: '11:00', endTime: '12:00' })
      .expect(200);
  });

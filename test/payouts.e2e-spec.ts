...
describe('Payouts — 시수 측정·페이 정산 (e2e)', () => {
  ...
  it('2) 수업 진행(held) + 보고서 작성·승인', async () => {
    ...
    await http.post(`/api/reports/${r1}/approve`).expect(201)
      .then((res) => expect(res.body.status).toBe('approved'));
    await http.post(`/api/reports/${r2}/approve`).expect(201);
    // S4 보고서는 제출만(미승인)
    await makeReport(S4);
    // S3 보고서 없음(작성하지 않음)
  });
  ...
  it('3) 수업 취소 발생 — S5는 보고서 승인돼도 취소면 시수 제외', async () => {
    ...
    await http.post(`/api/reports/${r5}/approve`).expect(201);
  ...
  it('5) 페이 산정(generate) — 정산서 생성 + 세션 연결, 값 일치', async () => {
    const p = (await http.post('/api/payouts/generate')
  ...
  it('7) 관리자 반려(reject) — 정산서 rejected + 세션 회수(재산정 가능)', async () => {
    ...
    const rejected = (await http.post(`/api/payouts/${target.id}/reject`).send({ reason: '근태 확인 필요' }).expect(201)).body;
  ...
  it('8) 관리자 급여 수정(adjust) — 자동 산정액 보존, 실효 지급액만 변경', async () => {
    const p = (await http.post('/api/payouts/generate')
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(201)).body;
    const adj = (await http.post(`/api/payouts/${p.id}/adjust`)
  ...
  it('9) 확정→지급(pay) — 통합 원장 출금 1줄 기록, 금액=실효 지급액', async () => {
    ...
    // 지급 전 확정 필요
    await http.post(`/api/payouts/${p.id}/pay`).expect(400); // confirmed 아니면 거부
    await http.post(`/api/payouts/${p.id}/confirm`).expect(201)

...
describe('Payouts — 시수 측정·페이 정산 (e2e)', () => {
  ...
  it('2) 수업 진행(held) + 보고서 작성·승인', async () => {
    ...
    await http.post(`/api/reports/${r1}/approve`).set(asAdmin()).expect(201)
  ...
  it('5) 페이 산정(generate) — 정산서 생성 + 세션 연결, 값 일치', async () => {
    const p = (await http.post('/api/payouts/generate').set(asAdmin())
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(201)).body;
    ...
    const s1 = (await http.get(`/api/schedule?from=${MON}&to=${SUN}`).expect(200)).body
      .find((r: { id: number }) => r.id === S1);
  ...
  it('6) 이중 계상 방지 — 같은 기간 재산정 시 적격 0 → generate 400', async () => {
    ...
    await http.post('/api/payouts/generate').set(asAdmin())
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(400);
  });
  ...
  it('7) 관리자 반려(reject) — 정산서 rejected + 세션 회수(재산정 가능)', async () => {
    ...
    const rejected = (await http.post(`/api/payouts/${target.id}/reject`).set(asAdmin()).send({ reason: '근태 확인 필요' }).expect(201)).body;
  ...
  it('8) 관리자 급여 수정(adjust) — 자동 산정액 보존, 실효 지급액만 변경', async () => {
    const p = (await http.post('/api/payouts/generate').set(asAdmin())
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(201)).body;
    const adj = (await http.post(`/api/payouts/${p.id}/adjust`).set(asAdmin())
      .send({ amount: 150000, reason: '교통비 차감' }).expect(201)).body;
  ...
  it('9) 확정→지급(pay) — 통합 원장 출금 1줄 기록, 금액=실효 지급액', async () => {
    ...
    await http.post(`/api/payouts/${p.id}/confirm`).set(asAdmin()).expect(201)
  ...
  it('FK: 존재하지 않는 sessionId로 보고서 작성 → 400', async () => {
    await http.post('/api/reports')
      .send({ sessionId: 999999, studentId: STUDENT, content: 'x' }).expect(400);
  });

  it('중복 방지: 같은 (세션·학생) 보고서 재작성 → 409', async () => {
    await http.post('/api/reports')
      .send({ sessionId: S1, studentId: STUDENT, content: '중복' }).expect(409);
  });
  ...
  it('인가: 비로그인으로 정산서 생성 → 401', async () => {
    await http.post('/api/payouts/generate')
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(401);
  });
  ...
  it('인가: 강사(instructor) 토큰으로 정산서 생성 → 403', async () => {
    ...
    await http.post('/api/payouts/generate').set({ Authorization: `Bearer ${login.body.accessToken}` })
      .send({ instructorId: INSTRUCTOR, from: MON, to: SUN }).expect(403);
  });
